...
try {
  ...
  for (const [name, route] of routes) {
    ...
    if (name === 'health-bmr') {
      ...
      await dispatchKey('rawKeyDown', 'ArrowUp', { code: 'ArrowUp', windowsVirtualKeyCode: 38 }); await dispatchKey('keyUp', 'ArrowUp', { code: 'ArrowUp', windowsVirtualKeyCode: 38 });
      await waitFor(`(() => {const input=document.querySelector('[data-testid="input-bmr-weight"]');const result=document.querySelector('[data-testid="result-bmr"]');return input?.value!==${JSON.stringify(before.value)}&&(result?.textContent||'')!==${JSON.stringify(before.result)};})()`, 'health-bmr trusted keyboard input/result reaction');
      interaction = await evaluate(`(() => {const input=document.querySelector('[data-testid="input-bmr-weight"]');const reset=document.querySelector('[data-testid="button-reset-bmr"]');const result=document.querySelector('[data-testid="result-bmr"]');if(!input||!reset||!(result?.textContent||'').trim())throw new Error('BMR input/result/reset missing after interaction');reset.focus();if(document.activeElement!==reset)throw new Error('BMR reset not focusable');return'ArrowUp trusted key changed BMR numeric value/result after hydration; reset focusable';})()`);
