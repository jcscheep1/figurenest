...
try {
  ...
  for (const [name, route] of routes) {
    ...
    if (name === 'health-bmr') {
      ...
      const nextValue = Number(before.value) === 80 ? '81' : '80';
      await dispatchKey('keyDown', 'a', { code: 'KeyA', windowsVirtualKeyCode: 65, modifiers: 2 });
      await dispatchKey('keyUp', 'a', { code: 'KeyA', windowsVirtualKeyCode: 65, modifiers: 2 });
      for (const digit of nextValue) {
        const windowsVirtualKeyCode = digit.charCodeAt(0);
        await dispatchKey('keyDown', digit, { code: `Digit${digit}`, windowsVirtualKeyCode, text: digit, unmodifiedText: digit });
        await dispatchKey('keyUp', digit, { code: `Digit${digit}`, windowsVirtualKeyCode });
      }
      await waitFor(`(() => {const input=document.querySelector('[data-testid="input-bmr-weight"]');const result=document.querySelector('[data-testid="result-bmr"]');return input?.value===${JSON.stringify(nextValue)}&&(result?.textContent||'')!==${JSON.stringify(before.result)};})()`, 'health-bmr trusted retype/result reaction');
      interaction = await evaluate(`(() => {const input=document.querySelector('[data-testid="input-bmr-weight"]');const reset=document.querySelector('[data-testid="button-reset-bmr"]');const result=document.querySelector('[data-testid="result-bmr"]');if(!input||!reset||!(result?.textContent||'').trim())throw new Error('BMR input/result/reset missing after interaction');reset.focus();if(document.activeElement!==reset)throw new Error('BMR reset not focusable');return'trusted Ctrl+A and numeric retype changed BMR value/result after hydration; reset focusable';})()`);
