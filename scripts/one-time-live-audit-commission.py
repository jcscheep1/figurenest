...
URL='https://figurenest.com/calculators/business/commission'
...
def rejected():
    ...
    return 'check the values' in text or 'valid range' in text

try:
    d.get(URL); wait.until(lambda x:x.execute_script('return document.readyState')=='complete'); time.sleep(.8); dismiss()
    assert 'Commission Calculator' in d.find_element(By.TAG_NAME,'h1').text
    fs=inputs(); assert len(fs)==3, len(fs)
    assert all(e.get_attribute('id') for e in fs)
    assert not d.find_elements(By.CSS_SELECTOR,'.calculator-mode'), 'Commission should not expose an unsupported Advanced mode'

    assert '$800.00' in out(), out()
    print('PASS default 10000 sales / 8% / 0 base -> $800.00')

    setv(fs[2],'')
    assert '$800.00' in out() and not rejected(), out()
    print('PASS blank optional base pay is treated as zero')

    setv(fs[0],'0'); setv(fs[1],'0'); setv(fs[2],'0')
    assert '$0.00' in out() and not rejected(), out()
    print('PASS true zero boundary remains valid')

    setv(fs[0],'100'); setv(fs[1],'100'); setv(fs[2],'0')
    assert '$100.00' in out() and not rejected(), out()
    print('PASS 100% commission-rate boundary -> $100.00')

    setv(fs[0],'')
    assert rejected(), page_text()
    assert fs[0].get_attribute('aria-invalid') == 'true'
    print('PASS blank required sales are rejected and exposed as aria-invalid')
    ...
    assert '/calculators/business/commission' in canonical, canonical
    schemas=[]
