...
const canonicalPublicUrls = [...new Set(publicRoutes.map((route) => toCanonicalUrl(route)))];
const expectedSitemapUrlCount = canonicalPublicUrls.length;
const expectedIndexablePageCount = publicRoutes.length;
...
for (const { route, html } of indexablePages) {
  ...
  if (pageText.length < substantiveTextFloor) failures.push(`${route}: server-rendered visible text is ${pageText.length} characters; ${substantiveTextFloor}-character floor required`);
  if (/\b51\s+(?:published\s+)?tools?\b|\b9\s+(?:active\s+)?categories?\b/i.test(pageText)) failures.push(`${route}: contains a stale legacy inventory total`);
  ...
  if (value(html, /<meta name="robots" content="([^"]+)"/i).toLowerCase() !== 'index, follow') failures.push(`${route}: robots must be index, follow`);
  ...
  if (value(html, /<meta property="og:image" content="([^"]+)"/i) !== `${productionOrigin}/social/figurenest-social-card.png`) failures.push(`${route}: missing absolute default Open Graph image`);
  ...
  if (value(html, /<meta name="twitter:image" content="([^"]+)"/i) !== `${productionOrigin}/social/figurenest-social-card.png`) failures.push(`${route}: missing absolute Twitter image`);
  ...
  if (!html.includes("ad_storage:'denied'") || !html.includes("ad_user_data:'denied'") || !html.includes("ad_personalization:'denied'")) failures.push(`${route}: missing denied-by-default Google consent state`);
  ...
  if (calculatorRoutes.has(route)) {
    ...
    for (const phrase of forbiddenBoilerplate) if (qualifyingCalculatorText(html).includes(phrase)) failures.push(`${route}: contains forbidden boilerplate: ${phrase}`);
    ...
    for (const image of images) {
      ...
      if (!/\bloading="lazy"/i.test(image) && !/\bfetchpriority="high"/i.test(image)) failures.push(`${route}: image must be lazy loaded or explicitly identified as the LCP image`);
    }
  }

  const moduleScripts = [...html.matchAll(/<script\b[^>]*>/gi)].map((match) => match[0]).filter((tag) => /\btype="module"/i.test(tag)).map((tag) => value(tag, /\bsrc="([^"]+)"/i)).filter(Boolean);
  ...
  for (const script of schemas) {
    try {
      ...
      if (route === '/') {
        if (!types.includes('Organization') || !types.includes('WebSite') || !types.includes('WebPage') || !types.includes('FAQPage')) failures.push('home: missing Organization, WebSite, WebPage, or FAQPage schema');
        ...
        if (organization?.['@id'] !== `${productionOrigin}/#organization` || typeof organization.logo !== 'object' || typeof organization.image !== 'object') failures.push('home: Organization schema is missing its stable identity, logo, or image');
      } else if (calculatorRoutes.has(route)) {
        const application = graph.find((node) => node['@type'] === 'WebApplication');
        if (!types.includes('WebApplication') || !types.includes('BreadcrumbList') || application?.url !== canonical || typeof application?.name !== 'string' || !application.name) failures.push(`${route}: missing or incorrect WebApplication or BreadcrumbList schema`);
        ...
        if (renderedFaqCount !== schemaFaqCount) failures.push(`${route}: ${renderedFaqCount} visible FAQs do not match ${schemaFaqCount} FAQ schema questions`);
        ...
        if (!article || !types.includes('Article') || !types.includes('BreadcrumbList') || !types.includes('FAQPage')) failures.push(`${route}: missing Article, BreadcrumbList, or FAQPage schema`);
        else {
          const articleNode = graph.find((node) => node['@type'] === 'Article');
          const organization = graph.find((node) => node['@type'] === 'Organization');
          if (!articleNode?.image || (articleNode.publisher as { '@id'?: string } | undefined)?.['@id'] !== `${productionOrigin}/#organization` || (articleNode.author as { '@id'?: string } | undefined)?.['@id'] !== `${productionOrigin}/#organization` || !organization?.logo) failures.push(`${route}: Article identity, image, publisher, or publisher logo is incomplete`);
          const faq = graph.find((node) => node['@type'] === 'FAQPage') as { mainEntity?: { name: string }[] } | undefined;
...
const obsoleteTimeDurationContent = ['09:00 to 17:30 on one date is 8 hours 30 minutes', 'GUIDANCE'];
for (const requiredText of requiredTimeDurationContent) if (!timeDurationText.includes(requiredText)) failures.push(`${timeDurationRoute}: production HTML is missing required calculator-specific content: ${requiredText}`);
for (const obsoleteText of obsoleteTimeDurationContent) if (timeDurationText.toLowerCase().includes(obsoleteText.toLowerCase())) failures.push(`${timeDurationRoute}: production HTML still contains obsolete content: ${obsoleteText}`);
if (/<div class="eyebrow">PRACTICAL GUIDE<\/div>/i.test(pageByRoute.get(timeDurationRoute) ?? '')) failures.push(`${timeDurationRoute}: production HTML still contains obsolete content: PRACTICAL GUIDE section`);
for (const phrase of forbiddenBoilerplate) if (timeDurationText.toLowerCase().includes(phrase.toLowerCase())) failures.push(`${timeDurationRoute}: production HTML contains forbidden boilerplate: ${phrase}`);
...
for (const route of trustRoutes) {
  ...
  if (/\bmailto:/i.test(html) || /[\w.+-]+@[\w.-]+\.[a-z]{2,}/i.test(text)) failures.push(`${route}: exposes a public email address instead of the established contact-form channel`);
}
...
for (const route of ['/about/', '/methodology/'] as const) {
  const html = pageByRoute.get(route) ?? '';
  if (!/contact form/i.test(visibleText(html)) || !/href="\/contact\/?"/i.test(html)) failures.push(`${route}: does not consistently direct public support to the contact form`);
}

const inboundCounts = new Map(publicRoutes.map((route) => [route, 0]));
for (const [source, targets] of internalLinkGraph) for (const target of targets) if (target !== source) inboundCounts.set(target, (inboundCounts.get(target) ?? 0) + 1);
for (const [route, inbound] of inboundCounts) if (inbound === 0 && !intentionalZeroInboundRoutes.has(normalizeRoutePath(route))) failures.push(`${route}: zero-inbound orphan in the rendered indexable-page graph`);
...
for (const definition of advancedCalculatorDefinitions) {
  ...
  for (const href of definition.relatedRoutes) {
    ...
    if (!link) failures.push(`${route}: related published tool is not rendered: ${href}`);
    else if (visibleText(link[1]) !== tool.name) failures.push(`${route}: related link ${href} must use the readable published tool name`);
  }
}
for (const tool of localTools) {
  ...
  if (!discoveryHtml || !discoveryHtml.includes(`href="${toPublicPath(tool.href)}"`)) failures.push(`${route}: calculator is orphaned from its category or hub page`);
  if ((inboundCounts.get(route) ?? 0) === 0) failures.push(`${route}: calculator has no inbound discovery in rendered HTML`);
  const contextualToolTargets = [...(internalLinkGraph.get(route) ?? [])].filter((target) => target !== route && calculatorRoutes.has(target));
  if (contextualToolTargets.length < 2) failures.push(`${route}: calculator has ${contextualToolTargets.length} outbound contextual tool links; at least 2 required`);
}
...
for (const matches of exactProse.values()) {
  ...
  if (routes.length > 1) failures.push(`duplicate calculator prose across ${routes.join(', ')}: "${matches[0].text.slice(0, 120)}"`);
  const perRoute = new Map<string, number>();
  for (const match of matches) perRoute.set(match.route, (perRoute.get(match.route) ?? 0) + 1);
  for (const [route, repetitions] of perRoute) if (repetitions > 1) failures.push(`${route}: repeats the same educational paragraph ${repetitions} times`);
}
...
for (let left = 0; left < longBlocks.length; left += 1) {
  ...
  for (let right = left + 1; right < longBlocks.length; right += 1) {
    ...
    if (similarity >= 0.85) failures.push(`near-duplicate calculator prose (${similarity.toFixed(2)}) across ${a.route} and ${b.route}: "${a.text.slice(0, 120)}"`);
  }
}
...
for (const category of localCategories) {
  ...
  if (!homeHtml.includes(`href="${destination}"`)) failures.push(`homepage/footer discovery is missing ${category.name}: ${destination}`);
}
if (/\b51\s+(?:published\s+)?tools?\b|\b9\s+(?:active\s+)?categories?\b/i.test(visibleText(homeHtml))) failures.push('homepage contains a stale legacy inventory total');
const initialJavaScript = localAssetPaths(homeHtml, 'js');
const initialJavaScriptBytes = (await Promise.all(initialJavaScript.map((asset) => fs.stat(path.join(output, asset.slice(1))).then((stat) => stat.size)))).reduce((total, size) => total + size, 0);
if (initialJavaScriptBytes > 530_000) failures.push(`performance: initial JavaScript is ${initialJavaScriptBytes} bytes; 530000-byte budget exceeded`);
const globalCss = localAssetPaths(homeHtml, 'css');
const globalCssBytes = (await Promise.all(globalCss.map((asset) => fs.stat(path.join(output, asset.slice(1))).then((stat) => stat.size)))).reduce((total, size) => total + size, 0);
if (globalCssBytes > 150_000) failures.push(`performance: global CSS is ${globalCssBytes} bytes; 150000-byte budget exceeded`);

for (const [route, marker] of [
  ['/articles', 'ArticlePages-'], ['/privacy', 'TrustPages-'], ['/home-construction', 'ConstructionPages-'], ['/calculators/finance/loan', 'advanced-calculator-pages-'], ['/calculators/math/average', 'advanced-calculator-pages-'], ['/calculators/technology/bandwidth', 'advanced-calculator-pages-'], ['/calculators/math/binary', 'advanced-calculator-pages-'], ['/calculators/health/bac', 'advanced-calculator-pages-'], ['/calculators/automotive/mileage', 'advanced-calculator-pages-'],
] as const) {
  const html = await fs.readFile(outputFile(route), 'utf8');
  const routeCss = localAssetPaths(html, 'css');
  if (!routeCss.some((asset) => asset.includes(marker))) failures.push(`${route}: prerendered HTML is missing its route stylesheet`);
}
if (localAssetPaths(homeHtml, 'css').some((asset) => /(?:ArticlePages|TrustPages|PrivateApp|construction-pages|advanced-calculator-pages)-/.test(asset))) failures.push('performance: homepage eagerly loads route-only CSS');
...
for (const privateRoute of ['sign-in', 'sign-up', 'control-center']) {
  ...
  if (!localAssetPaths(privateHtml, 'css').some((asset) => asset.includes('PrivateApp-'))) failures.push(`${privateRoute}: prerendered HTML is missing private route CSS`);
}
...
if (value(legacyConstruction, /<link rel="canonical" href="([^"]+)"/i) !== toCanonicalUrl('/home-construction')) failures.push('legacy construction redirect: incorrect canonical');
if (value(legacyConstruction, /<meta http-equiv="refresh" content="([^"]+)"/i) !== '0;url=/home-construction/') failures.push('legacy construction redirect: missing static redirect');
if (!legacyConstruction.includes('window.location.replace("/home-construction/" + window.location.search + window.location.hash)')) failures.push('legacy construction redirect: query-preserving browser redirect is missing');
if (!legacyConstruction.includes('<div id="root"></div><noscript>')) failures.push('legacy construction redirect: fallback is not hydration-safe');
...
if (sitemapUrls.length !== expectedSitemapUrlCount) failures.push(`sitemap: expected exactly ${expectedSitemapUrlCount} canonical URLs, found ${sitemapUrls.length}`);
...
if (sitemapUrls.some((url) => /\/(?:api|admin|account|auth|internal|private|control-center|sign-in|sign-up)(?:\/|$)/.test(url))) failures.push('sitemap: private route leaked into the public sitemap');
...
for (const url of canonicalPublicUrls) if (!sitemapUrls.includes(url)) failures.push(`sitemap: missing ${url}`);
...
const requiredRobotDirectives = ['Allow: /', 'Disallow: /api/', 'Disallow: /admin/', 'Disallow: /account/', 'Disallow: /auth/', 'Disallow: /internal/', 'Disallow: /private/', 'Disallow: /control-center/', 'Disallow: /sign-in/', 'Disallow: /sign-up/', `Sitemap: ${SITE_ORIGIN}/sitemap.xml`];
if (requiredRobotDirectives.some((directive) => !robots.includes(directive))) failures.push('robots.txt: incorrect crawl or sitemap directive');
for (const obsoletePath of legacyRedirectPaths) {
  const escapedPath = obsoletePath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  if (new RegExp(`^\\s*Disallow:\\s*${escapedPath}(?:/)?\\s*$`, 'm').test(robots)) failures.push(`robots.txt: obsolete redirect path is blocked: ${obsoletePath}`);
  if (sitemapUrls.some((url) => new URL(url).pathname === obsoletePath)) failures.push(`sitemap: obsolete redirect path is listed: ${obsoletePath}`);
}

const adsTxt = await fs.readFile(path.join(output, 'ads.txt'), 'utf8');
if (adsTxt.trim() !== ADSENSE_ADS_TXT_RECORD) failures.push('ads.txt: incorrect or missing Google AdSense authorization record');
const requiredOutputAssets = ['robots.txt', 'sitemap.xml', 'ads.txt', '404.html', 'favicon.svg', 'favicon.ico', 'favicon-16.png', 'favicon-32.png', 'favicon-64.png', 'figurenest-icon-96.png', 'figurenest-logo.svg', 'figurenest-icon-mono.svg', 'apple-touch-icon.png', 'manifest.webmanifest', 'site.webmanifest', 'pwa-192.png', 'pwa-512.png', 'pwa-maskable-512.png', 'pwa-maskable.svg', 'social/figurenest-social-card.png', 'social/figurenest-social-card.svg', 'service-worker.js', 'offline.html'];
...
console.log(`Built-site audit passed: ${publicRoutes.length} rendered indexable routes, ${expectedSitemapUrlCount} unique canonical sitemap URLs, ${localTools.length} directory tool guides checked for discovery plus ${publishedTools.length - localTools.length} indexable converter hub, ${Object.keys(categoryContent).length} expanded categories, and the custom 404`);
