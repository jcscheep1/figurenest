...
test('all site-wide tool and category counts derive from published tool pages', () => {
  ...
  assert.equal(
    localCategories.reduce((total, category) => total + category.toolCount, 0),
    publishedToolCount,
  );
  for (const category of localCategories) {
    assert.equal(
      ...
      `${category.name} has a stale published-tool count`,
    );
  }
...
test('every catalog category has one indexable discovery route in the sitemap', () => {
  const discoveryRoutes = localCategories.map((category) => (
    category.slug === 'construction' ? '/home-construction' : `/category/${category.slug}`
  ));
...
test('tool slugs, names, descriptions, and destinations are unique', () => {
  for (const values of [
    localTools.map((item) => item.slug),
    localTools.map((item) => item.name),
    localTools.map((item) => item.description),
    localTools.map((item) => item.href),
  ]) {
...
test('canonical related-tool graph covers every published tool with reciprocal semantic links', () => {
  ...
  const explicitCrossCategoryPairs = new Set(crossCategoryToolComplements.flatMap(([left, right]) => [
    `${left}:${right}`,
    `${right}:${left}`,
  ]));
  ...
  for (const source of publishedTools) {
    ...
    for (const target of related) {
      ...
      assert.ok(
        target.categorySlug === source.categorySlug
          || explicitCrossCategoryPairs.has(`${source.slug}:${target.slug}`),
        `${source.slug} has an uncurated cross-category link to ${target.slug}`,
      );
...
test('legacy and hostname redirects collapse into one query-preserving 301 target', () => {
  ...
  for (const [hostname, protocol, pathname, search, expected] of cases) {
    assert.equal(getRequestRedirect({ hostname, protocol, pathname, search }), expected);
  }
  assert.equal(getRequestRedirect({
    hostname: 'figurenest.com',
    protocol: 'https:',
    pathname: '/calculators/finance/loan/',
  }), null);
  assert.equal(getRequestRedirect({
    hostname: 'localhost',
    protocol: 'http:',
...
test('PWA metadata uses the FigureNest identity without changing public routes', () => {
  ...
  for (const icon of webManifest.icons) {
    assert.ok(fs.existsSync(new URL(`public/${icon.src.replace('./', '')}`, artifactRoot)), `missing PWA icon ${icon.src}`);
  }
});
...
test('PWA offline behavior is network-first and bypasses private, auth, and API requests', () => {
  ...
  assert.match(serviceWorker, /responseCanBeCached/);
  assert.match(serviceWorker, /private\|no-store/);

  const listeners = new Map<string, (event: { request: Record<string, string>; respondWith: () => void }) => void>();
  let networkRequests = 0;
  const workerSelf = {
    location: { origin: 'https://figurenest.com' },
    registration: { scope: 'https://figurenest.com/' },
    clients: { claim: async () => undefined },
    skipWaiting: async () => undefined,
    addEventListener: (type: string, listener: (event: { request: Record<string, string>; respondWith: () => void }) => void) => listeners.set(type, listener),
  };
  vm.runInNewContext(serviceWorker, {
    ...
    fetch: () => {
      networkRequests += 1;
      throw new Error('private requests must bypass the worker');
    },
  });
  const fetchListener = listeners.get('fetch');
  assert.ok(fetchListener);
  for (const path of ['/sign-in', '/sign-up', '/control-center', '/control-center/seo', '/api/session']) {
    let intercepted = false;
    fetchListener({
      request: {
        method: 'GET',
        url: `https://figurenest.com${path}`,
        mode: 'navigate',
        destination: 'document',
      },
      respondWith: () => { intercepted = true; },
    });
    assert.equal(intercepted, false, `${path} must bypass service-worker navigation caching`);
  }
  let apiImageIntercepted = false;
  fetchListener({
    request: {
      method: 'GET',
      url: 'https://figurenest.com/api/account/avatar',
      mode: 'cors',
      destination: 'image',
    },
    respondWith: () => { apiImageIntercepted = true; },
  });
  assert.equal(apiImageIntercepted, false);
  assert.equal(networkRequests, 0);
});

test('SEO accepts legacy slash variants and redirects the retired construction category', () => {
  assert.equal(getSeoForPath('/about').canonical, 'https://figurenest.com/about/');
  assert.equal(getSeoForPath('/about/').canonical, 'https://figurenest.com/about/');
  const legacy = getSeoForPath('/category/construction/');
  assert.equal(legacy.redirect, '/home-construction/');
  assert.equal(legacy.canonical, 'https://figurenest.com/home-construction/');
  assert.equal(legacy.robots, 'noindex, nofollow');
});

test('percentage change has dedicated indexable SEO and FAQ schema', () => {
  const seo = getSeoForPath('/calculators/math/percentage-increase-decrease');
  assert.equal(seo.title, 'Percentage Change Calculator | FigureNest');
  assert.equal(seo.canonical, 'https://figurenest.com/calculators/math/percentage-increase-decrease/');
  assert.equal(seo.robots, 'index, follow');
  assert.ok(seo.description.length >= 70 && seo.description.length <= 160);
  const schema = JSON.stringify(seo.schema);
  assert.match(schema, /WebApplication/);
  assert.match(schema, /BreadcrumbList/);
  assert.match(schema, /FAQPage/);
  assert.match(schema, /Why is percentage change undefined/);
});

test('date duration has dedicated indexable SEO and shared FAQ schema', () => {
  const seo = getSeoForPath('/calculators/date-time/date-difference');
  assert.equal(seo.title, 'Date Duration Calculator | FigureNest');
  assert.equal(seo.h1, 'Date Duration Calculator');
  assert.equal(seo.canonical, 'https://figurenest.com/calculators/date-time/date-difference/');
  assert.equal(seo.robots, 'index, follow');
  assert.ok(seo.description.length >= 70 && seo.description.length <= 160);
  assert.match(seo.description, /exact duration/i);
  assert.match(seo.description, /reversed dates/i);
  assert.match(seo.description, /leap years/i);
  const schema = JSON.stringify(seo.schema);
  assert.match(schema, /WebApplication/);
  assert.match(schema, /BreadcrumbList/);
  assert.match(schema, /FAQPage/);
  assert.match(schema, /Does the calculator include the end date/);
});

test('loan, mortgage, and compound interest have distinct indexable SEO and visible FAQ schema', () => {
  const expected = [
    ['loan', 'Loan Payment Calculator | FigureNest', 'How is a loan payment calculated'],
    ['mortgage', 'Mortgage Payment Calculator | FigureNest', 'What does the monthly mortgage estimate include'],
    ['compound-interest', 'Compound Interest Calculator | FigureNest', 'What is compound interest'],
  ] as const;

  for (const [slug, title, faqQuestion] of expected) {
    const path = `/calculators/finance/${slug}`;
    const seo = getSeoForPath(path);
    assert.equal(seo.title, title);
    assert.equal(seo.canonical, `https://figurenest.com${path}/`);
    assert.equal(seo.robots, 'index, follow');
    assert.ok(seo.description.length >= 100 && seo.description.length <= 160);
    const schema = JSON.stringify(seo.schema);
    assert.match(schema, /WebApplication/);
    assert.match(schema, /BreadcrumbList/);
    assert.match(schema, /FAQPage/);
    assert.match(schema, new RegExp(faqQuestion));
  }
});
...
test('age, working days, salary, and overtime have distinct indexable SEO and visible FAQ schema', () => {
  const expected = [
    ...
    ['/calculators/date-time/working-days', 'Working Days Calculator — Count Weekdays | FigureNest', 'Are the start and end dates included'],
  ...
  for (const [path, title, faqQuestion] of expected) {
    const seo = getSeoForPath(path);
    assert.equal(seo.title, title);
    assert.equal(seo.canonical, `https://figurenest.com${path}/`);
    assert.equal(seo.robots, 'index, follow');
...
test('core converter cluster has distinct indexable SEO and schema synchronized with visible FAQs', () => {
  for (const slug of converterSlugs) {
    ...
    assert.equal(seo.description, content.seoDescription);
    assert.equal(seo.h1, `${slug[0].toUpperCase()}${slug.slice(1)} Converter`);
    assert.equal(seo.canonical, `https://figurenest.com${path}/`);
    assert.equal(seo.robots, 'index, follow');
    const schema = JSON.stringify(seo.schema);
    assert.match(schema, /WebApplication/);
    ...
    assert.match(schema, /FAQPage/);
    for (const faq of content.faqs) {
      assert.ok(schema.includes(faq.question));
      assert.ok(schema.includes(faq.answer));
    }
  }
});
...
test('business calculator cluster has distinct indexable SEO and visible FAQ schema', () => {
  for (const slug of businessCalculatorSlugs) {
    const path = `/calculators/business/${slug}`;
    const content = businessCalculatorContent[slug];
    const seo = getSeoForPath(path);
    assert.equal(seo.title, content.seoTitle);
    assert.equal(seo.description, content.seoDescription);
    assert.equal(seo.h1, content.title);
    assert.equal(seo.canonical, `https://figurenest.com${path}/`);
    assert.equal(seo.robots, 'index, follow');
  ...
  const expected = [
    {
      ...
      categorySlug: 'math',
      path: '/calculators/math/percentage-increase-decrease/',
    },
    {
      slug: 'date-difference',
      categorySlug: 'date-time',
      path: '/calculators/date-time/date-difference/',
    },
  ];

  for (const item of expected) {
    const tool = publishedTools.find((candidate) => candidate.slug === item.slug);
    assert.ok(tool, `${item.slug} is missing from the published catalog`);
    assert.equal(tool.categorySlug, item.categorySlug);
    assert.ok(sitemapPaths.includes(item.path), `${item.slug} is missing from the sitemap`);
    assert.ok(
      localCategories.some((category) => category.slug === item.categorySlug && category.toolCount > 0),
      `${item.slug} has no internally linked category`,
    );
    const seo = getSeoForPath(tool.href);
    assert.equal(seo.robots, 'index, follow');
    assert.equal(seo.canonical, toCanonicalUrl(tool.href));
  }
});

test('every catalog destination matches its slug and route family', () => {
  for (const item of localTools) {
    const expected = item.href.startsWith('/converters/')
      ? `/converters/${item.slug}`
      : `/calculators/${item.categorySlug}/${item.slug}`;
    assert.equal(item.href, expected, `${item.name} has a mismatched destination`);
  }
});

test('construction related links all resolve to published construction tools', () => {
  const published = new Set(constructionTools.map((item) => item.slug));
  for (const item of constructionTools) {
    assert.ok(item.related.length > 0, `${item.name} has no related links`);
    for (const related of item.related) assert.ok(published.has(related), `${item.name} links to missing tool ${related}`);
  }
});

test('sitemap exactly matches all indexable app pages', () => {
  const staticPaths = ['/', '/calculators', '/converters/unit', '/home-construction', '/about', '/contact', '/privacy', '/cookies', '/terms', '/disclaimer', '/methodology', '/articles'].map(toPublicPath);
  const categoryPaths = localCategories
    .filter((category) => category.slug !== 'construction')
    .map((category) => toPublicPath(`/category/${category.slug}`));
  const articlePaths = articles.map((article) => toPublicPath(`/articles/${article.slug}`));
  const expected = [...staticPaths, ...categoryPaths, ...localTools.map((item) => toPublicPath(item.href)), ...articlePaths].sort();
  assert.deepEqual([...sitemapPaths].sort(), expected);
...
test('every literal internal link points to an indexable route or the home search anchor', () => {
  const sourceFiles = [
    'src/components/FigureNestShell.tsx',
    'src/pages/AppPages.tsx',
    'src/pages/TrustPages.tsx',
    'src/pages/ConstructionPages.tsx',
    'src/pages/ArticlePages.tsx',
    'src/pages/FinanceCalculatorPage.tsx',
  ];
  const allowed = new Set([...sitemapPaths, '/#search']);
  for (const path of sourceFiles) {
    const source = fs.readFileSync(new URL(path, artifactRoot), 'utf8');
    const hrefs = [...source.matchAll(/href="(\/[^"]*)"/g)].map((match) => match[1]);
    for (const href of hrefs) assert.ok(allowed.has(toPublicPath(href)), `${path} links to an unindexed or missing route: ${href}`);
  }
});
...
test('article registry is substantial, unique, connected, and metadata-safe', () => {
  ...
  for (const values of [
    ...
    articles.map((article) => article.seoTitle),
    articles.map((article) => article.metaDescription),
  ]) assert.equal(new Set(values).size, articles.length);

  const slugs = new Set(articles.map((article) => article.slug));
  const calculatorPaths = new Set(publishedTools.map((tool) => tool.href));
  for (const article of articles) {
    assert.ok(article.seoTitle.length <= 60, `${article.slug} title is too long`);
    assert.ok(article.metaDescription.length >= 70 && article.metaDescription.length <= 160, `${article.slug} description length is unsafe`);
    assert.ok(article.sections.length >= 4, `${article.slug} needs substantive sections`);
    assert.equal(article.faqs.length, 4, `${article.slug} needs four visible FAQs`);
    assert.ok(article.assumptions.length >= 3 && article.limitations.length >= 3 && article.nextSteps.length >= 3);
    assert.ok(article.sections.some((section) => section.formula), `${article.slug} needs a visible formula`);
    assert.ok(article.sections.some((section) => section.example), `${article.slug} needs a worked example`);
    for (const related of article.relatedArticles) assert.ok(slugs.has(related), `${article.slug} links to missing article ${related}`);
    for (const calculator of article.relatedCalculators) assert.ok(calculatorPaths.has(calculator.href), `${article.slug} links to missing calculator ${calculator.href}`);
  }
});

test('article worked examples match their stated formulas and rounding convention', () => {
  const fixedPayment = (principal: number, annualRate: number, payments: number) => {
    const periodicRate = annualRate / 12;
    return principal * periodicRate / (1 - (1 + periodicRate) ** -payments);
  };
  const articleText = (slug: string) => JSON.stringify(articles.find((article) => article.slug === slug));

  const mortgagePayment = fixedPayment(320_000, 0.06, 360);
  assert.equal(mortgagePayment.toFixed(2), '1918.56');
  assert.equal((Number(mortgagePayment.toFixed(2)) * 360).toFixed(2), '690681.60');
  assert.equal((Number(mortgagePayment.toFixed(2)) * 360 - 320_000).toFixed(2), '370681.60');
  assert.match(articleText('mortgage-payment-basics'), /690,681\.60/);
  assert.match(articleText('mortgage-payment-basics'), /370,681\.60/);

  const loanPayment = fixedPayment(18_000, 0.072, 48);
  assert.equal(loanPayment.toFixed(2), '432.70');
  assert.equal((Number(loanPayment.toFixed(2)) * 48).toFixed(2), '20769.60');
  assert.equal((Number(loanPayment.toFixed(2)) * 48 - 18_000).toFixed(2), '2769.60');
  assert.match(articleText('loan-payment-calculations'), /20,769\.60/);
  assert.match(articleText('loan-payment-calculations'), /2,769\.60/);

  const refinancingBreakEvenMonths = 4_800 / (1_850 - 1_690);
  assert.equal(refinancingBreakEvenMonths, 30);
  assert.match(articleText('refinancing-considerations'), /30 months/);

  const compoundBalance = 10_000 * (1 + 0.04 / 12) ** (12 * 5);
  assert.equal(compoundBalance.toFixed(2), '12209.97');
  assert.equal((compoundBalance - 10_000).toFixed(2), '2209.97');
  assert.match(articleText('compound-interest-guide'), /12,209\.97/);
  assert.match(articleText('compound-interest-guide'), /2,209\.97/);
});

test('article and homepage schemas stay aligned with visible FAQ registries', () => {
  const home = getSeoForPath('/');
  const homeGraph = (home.schema as { '@graph': Record<string, unknown>[] })['@graph'];
  const homeFaq = homeGraph.find((node) => node['@type'] === 'FAQPage') as { mainEntity: { name: string; acceptedAnswer: { text: string } }[] };
  assert.deepEqual(homeFaq.mainEntity.map((item) => [item.name, item.acceptedAnswer.text]), homepageFaqData.map((item) => [item.question, item.answer]));
  const homeOrganization = homeGraph.find((node) => node['@type'] === 'Organization') as Record<string, unknown>;
  assert.equal(homeOrganization['@id'], 'https://figurenest.com/#organization');
  assert.ok(homeOrganization.logo);
  assert.ok(homeOrganization.image);

  const indexTypes = ((getSeoForPath('/articles').schema as { '@graph': { '@type': string }[] })['@graph']).map((node) => node['@type']);
  assert.ok(indexTypes.includes('CollectionPage'));
  assert.ok(indexTypes.includes('BreadcrumbList'));

  for (const article of articles) {
    const seo = getSeoForPath(`/articles/${article.slug}`);
    assert.equal(seo.title, article.seoTitle);
    assert.equal(seo.description, article.metaDescription);
    assert.equal(seo.status, 200);
    const graph = (seo.schema as { '@graph': Record<string, unknown>[] })['@graph'];
    const types = graph.map((node) => node['@type']);
    assert.ok(types.includes('Article'));
    assert.ok(types.includes('BreadcrumbList'));
    assert.ok(types.includes('FAQPage'));
    assert.ok(types.includes('Organization'));
    const articleSchema = graph.find((node) => node['@type'] === 'Article') as Record<string, unknown>;
    assert.ok(articleSchema.image);
    assert.deepEqual(articleSchema.publisher, { '@id': 'https://figurenest.com/#organization' });
    assert.deepEqual(articleSchema.author, { '@id': 'https://figurenest.com/#organization' });
    const faq = graph.find((node) => node['@type'] === 'FAQPage') as { mainEntity: { name: string; acceptedAnswer: { text: string } }[] };
    assert.deepEqual(faq.mainEntity.map((item) => [item.name, item.acceptedAnswer.text]), article.faqs.map((item) => [item.question, item.answer]));
  }
  assert.equal(getSeoForPath('/articles/not-a-real-guide').status, 404);
  assert.equal(getSeoForPath('/articles/not-a-real-guide').robots, 'noindex, follow');
});

test('homepage guide previews cover the complete article collection', () => {
  assert.deepEqual(
    homepageArticlePreviews.map((preview) => preview.href).sort(),
    articles.map((article) => `/articles/${article.slug}`).sort(),
  );
});
...
test('user-facing sources contain no retired demo or preview content', () => {
  const files = [
    ...
    'public/sitemap.xml',
    '../api-server/src/routes/catalog.ts',
  ];
  const content = files.map((path) => fs.readFileSync(new URL(path, artifactRoot), 'utf8')).join('\n');
  for (const retired of [
    /READ-ONLY PREVIEW/i,
    /ADMIN FOUNDATION/i,
    /calculations today/i,
    /EST\. 2025/i,
    /2025-02-18/i,
    /12847/,
    /href="\/admin"/i,
    /category\/(?:health-fitness|travel|everyday-tools)/i,
    new RegExp(['s', 'olve', 'ora'].join(''), 'i'),
  ]) {
    assert.doesNotMatch(content, retired);
  }
});