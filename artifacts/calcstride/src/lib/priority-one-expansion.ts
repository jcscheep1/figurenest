...
export function calculatePriorityOneExpansion(slug: PriorityOneExpansionSlug, values: string[], currency: PriorityOneCurrencyCode = 'USD'): PriorityOneResult {
 ...
 if(['boat-loan','business-loan','home-equity-loan','personal-loan','student-loan','uk-mortgage','canadian-mortgage'].includes(slug)){let r=n[1];if(slug==='canadian-mortgage')r=(Math.pow(1+r/200,2/12)-1)*1200;const m=payment(n[0],r,n[2]);return {primary:money(m),summary:'Estimated monthly principal and interest.',details:[{label:'Total payments',value:money(m*n[2]*12)},{label:'Total interest',value:money(m*n[2]*12-n[0])}]};}
 ...
 if(slug==='investment'){const end=n[0]*Math.pow(1+n[1]/1200,n[2]*12);return {primary:money(end),summary:'Compound-growth projection.',details:[{label:'Growth',value:money(end-n[0])}]};}
 ...
 if(slug==='va-mortgage'){const loan=(n[0]-n[1])*(1+n[4]/100);return {primary:money(payment(loan,n[2],n[3])),summary:'Estimated principal and interest.',details:[{label:'Financed funding fee',value:money(loan-(n[0]-n[1]))}]};}
