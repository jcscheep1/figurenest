...
export function calculatePriorityOneExpansion(slug: PriorityOneExpansionSlug, values: string[], currency: PriorityOneCurrencyCode = 'USD'): PriorityOneResult {
 ...
 if(slug==='pace'){if(!n[0])return bad('Distance must be greater than zero.');const totalSeconds=Math.round(n[1]/n[0]*60),paceMinutes=Math.floor(totalSeconds/60),paceSeconds=totalSeconds%60;return {primary:`${paceMinutes}:${String(paceSeconds).padStart(2,'0')} per km`,summary:'Average pace.',details:[]};}
