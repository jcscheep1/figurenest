...
const specs: Array<[PriorityOneExpansionSlug, string, ExpansionCategory, string, string, PriorityOneField[]]> = [
 ...
 ['estate-tax','Estate Tax Calculator','finance','Plan an estimate after an entered exemption and rate.','taxable estate = max(0, estate − exemption)',[f('estate','Estate value','15000000'),f('exemption','Exemption','13610000'),{...f('rate','Marginal rate','40','%'),max:100}]],
 ...
 ['personal-loan','Personal Loan Calculator','finance','Estimate a fixed personal loan payment.','monthly amortization formula',[moneyFields[0],moneyFields[1],{...moneyFields[2],min:1/12,step:String(1/12)}]],
...
export function calculatePriorityOneExpansion(slug: PriorityOneExpansionSlug, values: string[], currency: PriorityOneCurrencyCode = 'USD'): PriorityOneResult {
 ...
 if(['boat-loan','business-loan','home-equity-loan','personal-loan','student-loan','uk-mortgage','canadian-mortgage'].includes(slug)){if(slug==='home-equity-loan'||slug==='personal-loan'){if(n[2]<=0)return bad('Loan term must be greater than zero.');const rawMonths=n[2]*12,months=Math.round(rawMonths);if(Math.abs(rawMonths-months)>1e-9)return bad('Loan term must resolve to a whole number of months.');}let r=n[1];if(slug==='canadian-mortgage')r=(Math.pow(1+r/200,2/12)-1)*1200;const m=payment(n[0],r,n[2]);if(!Number.isFinite(m))return bad('These loan terms do not produce a finite payment.');return {primary:money(m),summary:'Estimated monthly principal and interest.',details:[{label:'Total payments',value:money(m*n[2]*12)},{label:'Total interest',value:money(m*n[2]*12-n[0])}]};}
 ...
 if(slug==='estate-tax'){if(n[2]>100)return bad('Marginal rate must be 100% or less.');const taxable=Math.max(0,n[0]-n[1]);return {primary:money(taxable*n[2]/100),summary:'Planning estimate using your entered exemption and rate.',details:[{label:'Taxable estate',value:money(taxable)}]};}
