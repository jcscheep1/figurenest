...
test('phase four known outputs and invalid input handling are deterministic', () => {
  const expected: Record<string, string> = {
    mileage: '30.00 MPG', 'roman-numeral': 'MMXXVI', 'shoe-size': 'US adult 9.0',
  ...
  assert.equal(calculatePhaseFour('tire-size', ['225', '45', '17', '25']).primary, '24.97 in diameter');
  ...
  assert.ok(calculatePhaseFour('shoe-size', ['JP', '9']).error);
