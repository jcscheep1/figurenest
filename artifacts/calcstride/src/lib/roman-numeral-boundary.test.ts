...
test('Roman numeral converter accepts only whole numbers from 1 through 3999', () => {
  ...
  for (const value of ['1.5', '2026.25', '3998.9']) {
    const result = calculatePhaseFour('roman-numeral', [value]);
    assert.ok(result.error, `${value} must be rejected rather than silently converted`);
  }

  for (const value of ['0', '4000', '', 'NaN', 'Infinity']) {
    assert.ok(calculatePhaseFour('roman-numeral', [value]).error, `${value || 'blank'} must be rejected`);
  }
});
