...
export function PhaseFourCalculatorPage({ slug }: { slug: PhaseFourSlug }) {
  ...
  return <Shell><Seo path={definition.href} /><div className="advanced-calc-page" data-testid={`page-${slug}`}>
    ...
    {result.details.length>0&&<div className="advanced-breakdown">{result.details.map((item)=><div key={item.label}><span>{item.label}</span><strong>{item.value}</strong></div>)}</div>}{(slug==='budget'||slug==='credit-card')&&<CalculatorDecisionExpansion slug={slug} values={values}/>}<button type="button" className="reset-button mt-6" onClick={()=>{setValues(defaults());setCopied(false);}}>Reset values</button></section></div>
