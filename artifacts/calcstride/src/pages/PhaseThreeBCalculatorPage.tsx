...
export function PhaseThreeBCalculatorPage({ slug }: { slug: PhaseThreeBSlug }) {
  ...
  const update = (index: number, value: string) => {
    if (slug === 'probability' && index === 0 && value in phaseThreeBProbabilityDefaults) {
      setValues([...phaseThreeBProbabilityDefaults[value as keyof typeof phaseThreeBProbabilityDefaults]]);
      setCopied(false);
      return;
    }
    ...
    <div className="advanced-calc-page" data-testid={`page-${slug}`}>
      <div className="advanced-calc-layout">
        <header className="advanced-calc-copy">
          <div className="eyebrow"><span className="eyebrow-dot" /> FIGURENEST MATH &amp; STATISTICS</div>
          <h1>{definition.h1}<span>.</span></h1>
          <p>{definition.description}</p>
          <p className="advanced-local-note">This calculator runs locally in your browser. Entered values are not sent to a calculation service.</p>
        </header>
      ...
      <div className="advanced-content-grid">
        ...
        <aside><div className="related-tools"><div className="eyebrow">RELATED TOOLS</div>{relatedTools.map((tool) => <Link href={tool.href} key={tool.href} className="advanced-aside-link">{tool.name}</Link>)}<Link href={categoryHref} className="advanced-aside-link">All Math tools</Link></div></aside>
      </div>
    </div>
  </Shell>;
}