...
export function PriorityOneCalculatorPage({ slug }: { slug: PriorityOneExpansionSlug }) {
  ...
  return <Shell>
    ...
    <div className="advanced-calc-page" data-testid={`page-${slug}`}>
      <nav aria-label="Breadcrumb" className="mono" data-testid={`breadcrumbs-${slug}`}>
        <Link href="/">Home</Link> / <Link href={`/category/${definition.categorySlug}`}>{definition.category}</Link> / {definition.name}
      </nav>
      <div className="advanced-calc-layout">
        <header className="advanced-calc-copy">
          ...
          <h1>{definition.name}<span>.</span></h1><p>{definition.description}</p>
        </header>
      ...
      <div className="advanced-content-grid"><article className="advanced-content">
        <section><div className="eyebrow">METHOD</div><h2>Formula and practical context.</h2><div className="advanced-formula-list"><div><strong>Formula</strong><code>{definition.formula}</code></div></div></section>
        {definition.educationalSections.map(section => <section key={section.heading}><div className="eyebrow">GUIDANCE</div><h2>{section.heading}</h2>{section.body.split('\n\n').map(paragraph => <p key={paragraph}>{paragraph}</p>)}</section>)}
        <section><div className="eyebrow">LIMITATIONS</div><h2>Check the assumptions.</h2><p>{definition.limitations}</p></section>
        {definition.sourceLinks.length > 0 && <section><div className="eyebrow">SOURCES</div><h2>Official context and guidance.</h2>{definition.sourceLinks.map(source => <p key={source.href}><a href={source.href} target="_blank" rel="noreferrer">{source.label}</a></p>)}</section>}
      ...
      </article><aside><div className="related-tools"><div className="eyebrow">RELATED TOOLS</div>{relatedTools.map(tool => <Link href={tool.href} key={tool.href} className="advanced-aside-link" data-testid={`link-related-${slug}-${tool.href.replaceAll('/','-')}`}>{tool.name}</Link>)}</div></aside></div>
    </div>
  </Shell>;
}