...
const pages: Record<TrustPageKind, Page> = {
  about: {
    ...
    sections: [
      { heading: 'What we make', body: <>Our tools cover common questions about money, work, home projects, vehicles, measurements, dates, printing, health, education, science, technology, and file tasks. A result is most useful when its inputs and limits are understandable, so we aim to keep each task focused instead of hiding it behind unnecessary steps. We also publish supporting guides and category explanations so visitors can understand which tool fits the question before relying on a number.</> },
      { heading: 'How to use the site', body: <>Start with the calculator or converter that matches your question, enter values that reflect your situation, and review the result alongside its units and assumptions. Treat the output as an estimate unless the page clearly says otherwise. Check worked examples, limitations, and related tools when they are provided, and compare important results with current documents or authoritative sources. For a fuller look at how the tools are checked and what they cannot tell you, read our <Link href="/methodology">editorial and calculator methodology</Link>.</> },
      { heading: 'How we build useful pages', body: <>A FigureNest page is intended to do more than return a number. Public tools should explain their purpose, the method or conversion relationship behind the result, important assumptions, practical examples, limitations, and nearby tools that answer related questions. Category and directory pages are designed as navigation and learning destinations rather than thin lists. We review public pages for clear titles, canonical routes, useful internal links, and consistent presentation on desktop and mobile.</> },
      { heading: 'Who operates the site', body: <>FigureNest is the publisher and operator of this independent calculator website. We are not a bank, insurer, medical provider, engineering practice, government agency, or professional advisory service, and publication of a calculator does not create that kind of relationship. The <Link href="/contact">contact form</Link> is our public support channel for questions, corrections, privacy requests, and tool suggestions. It is not an emergency or professional-advice service.</> },
      { heading: 'Privacy and responsible use', body: <>Public calculator inputs are designed to be processed without requiring an account. Browser-only file tools state their local-processing behavior and limitations on the tool page. Visitors should still avoid entering confidential or sensitive information unless it is genuinely necessary. Our <Link href="/privacy">Privacy Policy</Link> and <Link href="/cookies">Cookie Policy</Link> explain analytics, advertising, consent, storage, and the choices available to visitors.</> },
      { heading: 'Feedback and corrections', body: <>We welcome reports of unclear wording, broken tools, missing assumptions, accessibility problems, or suspected calculation errors. Please include the page, inputs, units, expected result if known, device or browser details when relevant, and a short explanation through our {contactLink()}. We review reproducible reports against the displayed method and update confirmed errors or unclear instructions when appropriate. Suggestions may also inform future calculator, converter, and guide priorities.</> },
    ],
  },
  contact: {
    label: 'Contact',
  ...
  terms: {
    label: 'Terms',
    title: 'Terms of Use',
    intro: 'These terms describe the basic rules for using FigureNest’s public calculators, converters, file tools, guides, and information.',
    sections: [
      { heading: 'Using FigureNest', body: <>You may use the public site for lawful, personal, educational, or business planning purposes. Use the tools in a way that respects other visitors and the service. Do not interfere with availability, attempt to bypass access controls, probe private areas, introduce malicious code, overload the service, misuse automated access, or use FigureNest in a way that harms others or violates applicable law.</> },
      { heading: 'Calculator and converter outputs', body: <>FigureNest provides general information, calculations, conversions, and estimates rather than individualized professional advice. Results depend on the inputs, units, formulas, assumptions, and rounding described on the relevant page. You are responsible for checking that the tool matches your question and that the values entered are appropriate. A result should not be treated as a promise, quotation, certification, approval, or instruction from a regulated professional.</> },
      { heading: 'File tools and local processing', body: <>Some public file tools are designed to process supported documents or images locally in the browser. The applicable tool page explains supported formats, resource limits, privacy behavior, and fidelity limitations. Local processing does not guarantee that every file can be opened or converted correctly. Keep an original copy of important files, inspect exported results before relying on them, and do not use a tool to process material you are not authorized to handle.</> },
      { heading: 'Content and availability', body: <>We may add, revise, correct, remove, rename, consolidate, or discontinue calculators, converters, guides, categories, routes, and other features as the site develops. FigureNest does not guarantee that the service will be continuously available, uninterrupted, error-free, compatible with every browser or device, or suitable for every purpose. Maintenance, security work, third-party service changes, or technical failures may temporarily affect availability.</> },
      { heading: 'Responsible automated access', body: <>Ordinary search-engine crawling and reasonable public-page access are expected, but automated activity must not impair the service, evade technical controls, imitate abusive traffic, scrape private areas, or generate disproportionate load. If an integration or research workflow needs unusually high-volume access, contact us first so we can assess a safe approach.</> },
      { heading: 'Your decisions and third-party information', body: <>You are responsible for checking inputs, assumptions, units, dates, rates, local rules, and results before relying on them. External services, advertisements, references, or links may be operated by third parties under their own terms and policies; their presence does not make FigureNest responsible for their content or decisions. Review the <Link href="/disclaimer">Disclaimer</Link> and <Link href="/methodology">methodology</Link> before using a result for a material decision.</> },
      { heading: 'Changes, corrections, and contact', body: <>These terms may be revised when FigureNest features, operating practices, or legal requirements change. The effective date shown on this page identifies the version currently published. We may correct obvious errors without advance notice. Submit questions about these terms, accessibility issues, or suspected misuse through the public {contactLink()}.</> },
    ],
  },
  disclaimer: {
    label: 'Disclaimer',
    ...
    sections: [
      { heading: 'Estimates have limits', body: <>A result depends on the values, formula, rounding, units, and assumptions used by a tool. Simplified calculators cannot represent every contract term, tax rule, medical circumstance, engineering condition, construction detail, market movement, product specification, or local requirement. Taxes, fees, rates, inflation, material waste, site conditions, eligibility rules, tolerances, weather, and real-world timing can materially change an outcome.</> },
      { heading: 'Verify important decisions', body: <>Use results as a starting point and verify them with current documents, measurements, quotes, codes, official guidance, product documentation, or an appropriate qualified professional when the stakes are high. FigureNest does not provide financial, investment, insurance, legal, tax, medical, engineering, construction, automotive, electrical, or other professional advice. A calculator result does not replace an inspection, diagnosis, quotation, eligibility decision, code review, or safety assessment.</> },
      { heading: 'Health, safety, and regulated decisions', body: <>Health, electrical, construction, vehicle, and similar tools may be useful for education or preliminary planning, but they cannot evaluate all personal, environmental, structural, or regulatory factors. Do not delay urgent medical care, bypass a required safety device, size regulated work solely from a calculator, or ignore manufacturer and local-authority instructions because a FigureNest estimate appears reasonable.</> },
      { heading: 'Money, rates, and future outcomes', body: <>Finance and business tools may use rates, terms, prices, contributions, tax assumptions, or growth inputs supplied by you. They do not predict markets, lending decisions, investment returns, tax treatment, credit outcomes, or future prices. Currency selectors generally change denomination and formatting rather than live exchange rates unless a page explicitly states otherwise. Confirm current rates, fees, rules, and contractual terms independently.</> },
      { heading: 'File conversion and data fidelity', body: <>Browser file tools may have format, memory, font, metadata, layout, color, transparency, animation, spreadsheet, or document-fidelity limitations. A successful download does not prove that every element was preserved. Keep the original file, inspect the result in an appropriate independent viewer or application, and do not assume an exported document is legally, visually, or technically equivalent to its source.</> },
      { heading: 'Availability and third-party material', body: <>FigureNest pages, referenced sources, advertisements, external links, and browser capabilities can change. We do not control third-party sites or guarantee that external material remains current, available, or suitable. When a decision depends on a law, rate, standard, product specification, timetable, or other changing fact, verify the current authoritative source before acting.</> },
      { heading: 'No promise of results', body: <>We work to make formulas, explanations, and limitations clear, but we do not guarantee accuracy, completeness, availability, savings, eligibility, compliance, fitness for a particular purpose, or any particular outcome. See the <Link href="/methodology">methodology</Link> for calculator quality checks and known limitations, or report a suspected error through our {contactLink()} with enough detail for us to reproduce it.</> },
    ],
  },
  methodology: {
    label: 'Methodology',
